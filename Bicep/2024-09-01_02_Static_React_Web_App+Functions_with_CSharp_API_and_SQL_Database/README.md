...
- `Static React Web App + Functions with C# API and SQL Database` in [awesome-azd](https://azure.github.io/awesome-azd/?tags=dotnetCsharp)
- GitHub, [todo-csharp-sql-swa-func](https://github.com/Azure-Samples/todo-csharp-sql-swa-func)

```shell
azd init -t todo-csharp-sql-swa-func
```

`location`が`japaneast`だとエラーになる。
少なくとも`AZURE_LOCATION="eastus2"`ならエラーにならないためロケーションは`eastus2`で構築すること。
