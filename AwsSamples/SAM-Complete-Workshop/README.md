...
```shell
sam init -n SAM-Complete-Workshop \
  --architecture x86_64 \
  --runtime dotnet6 \
...
```shell
aws codecommit create-repository --repository-name SAM-Complete-Workshop
aws codecommit delete-repository --repository-name SAM-Complete-Workshop
````

### アプリケーション削除

```shell
sam delete --stack-name SAM-Complete-Workshop
```

### デプロイ(`sync`)しながらの開発

```shell
sam sync --watch --stack-name SAM-Complete-Workshop
```

```shell
sam logs --tail --stack-name SAM-Complete-Workshop --name HelloWorldFunction
```

### ローカル実行

- [参考](https://catalog.workshops.aws/complete-aws-sam/en-US/module-2-local/20-run)
- 個々の`Lambda`関数を呼び出し：[sam local invoke reference](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-local-invoke.html)

```shell
sam local invoke --event events/event.json
```

- `APIGateway`をシミュレートするローカルHTTPサーバーを実行
  - `C#`や`Java`などのコンパイルが必要な場合は`sam build`が必要な模様

```shell
sam build && sam local start-api --port 8080
```
...
```bash
SAM-Complete-Workshop$ sam build
```

The SAM CLI installs dependencies defined in `src/HelloWorld.csproj`, creates a deployment package, and saves it in
...
```bash
SAM-Complete-Workshop$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
SAM-Complete-Workshop$ sam local start-api
SAM-Complete-Workshop$ curl http://localhost:3000/
```
...
```bash
SAM-Complete-Workshop$ sam logs -n HelloWorldFunction --stack-name SAM-Complete-Workshop --tail
```
...
```bash
SAM-Complete-Workshop$ dotnet test test/HelloWorld.Test
```
