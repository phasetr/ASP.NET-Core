...
type Msg =
  ...
  | ArticleFavored of RemoteData<string list, FullArticle>
  | ArticleUnfavored of RemoteData<string list, FullArticle>
...
let init session =
  ...
  { Articles = Loading
    ...
    ArticlesView = articlesView
    Session = session },
  Cmd.batch [ fetchArticles session articlesView 1; fetchTags ]
...
let update msg model : Model * Cmd<Msg> =
  ...
  | SetArticlesPage page ->
    ...
    fetchArticles model.Session model.ArticlesView page
  | TagsFetched data -> { model with PopularTags = data }, Cmd.none
  | ArticleFavored(Success article) ->
    map
  ...
  | ArticleFavored _ -> model, Cmd.none
  | ArticleUnfavored(Success article) ->
    map
      (fun (articles: ArticlesList) ->
        let updatedArticles =
          List.map
  ...
  | ToggleArticlesView articlesView ->
    { model with
        ArticlesView = articlesView },
    fetchArticles model.Session articlesView 1
      ...
      (fun (Tag tag) ->
        a
          [ ClassName "tag-pill tag-default"
            ...
            OnClick(fun ev ->
              ev.preventDefault ()
              Tag tag |> TagFeed |> ToggleArticlesView |> dispatch) ]
          [ str tag ])
      tags)
  ...
  ul
    ...
    (seq {
      for i in 1 .. totalArticles / 10 ->
        li
          [ classList [ ("page-item", true); ("active", i = currentPage) ] ]
          [ a
              [ ClassName "page-link"
                Href "#"
                OnClick(fun ev ->
             ...
             li
               ...
               [ a
                   [ classList
                     ...
                     OnClick(fun ev ->
                       ev.preventDefault ()
                       dispatch <| ToggleArticlesView Feed) ]
                   [ str "Your Feed" ] ]
           | None -> empty)
          li
            [ ClassName "nav-item" ]
            [ a
                [ classList
                  ...
                  OnClick(fun ev ->
                    ev.preventDefault ()
                    dispatch <| ToggleArticlesView Global) ]
                [ str "Global Feed" ] ]
