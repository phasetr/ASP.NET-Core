...
let private fetchArticle session slug =
  ...
  | Some s ->
    Cmd.OfAsync.perform
      Articles.fetchArticleWithSession
      {| Session = s; Slug = slug |}
      ArticleFetched
...
let init session slug =
  ...
  { Article = Loading
    ...
    Errors = []
    Authentication = authentication },
  Cmd.batch [ fetchArticle session slug; fetchComments slug; cmdUser ]
...
let private commentForm dispatch (user: User) errors newComment =
  form
    [ ClassName "card comment-form"; OnSubmit(fun _ -> dispatch SubmitComment) ]
    [ errorsList errors
      div
        [ ClassName "card-block" ]
        [ textarea
...
let private comments dispatch model =
  div
    [ ClassName "row" ]
    [ div
        [ ClassName "cols-xs-12 col-md-8 offset-md-2" ]
        [ (match model.Authentication with
           | Authenticated { User = (Success user) } ->
             commentForm dispatch user model.Errors model.NewComment
           | _ ->
             p
               []
...
let private articleOwnerButtons dispatch article =
  fragment
    ...
    [ a
        ...
        [ i [ ClassName "ion-edit" ] []; str " Edit Article" ]
      str " "
      button
        [ ClassName "btn btn-outline-danger btn-sm"
          OnClick(fun _ -> dispatch <| DeleteArticle article) ]
        [ i [ ClassName "ion-trash-a" ] []; str " Delete Article" ] ]
...
let private infoButtons dispatch authentication (article: FullArticle) =
  match authentication with
  | Authenticated auth when auth.Session.Username = article.Author.Username ->
    articleOwnerButtons dispatch article
  | Authenticated _ ->
    fragment
      []
