...
let loadStoryItem (itemId: int) =
  async {
    let endpoint = $"https://hacker-news.firebaseio.com/v0/item/%d{itemId}.json"
...
let render (state: State) (_: Msg -> unit) =
  Html.div
    [ prop.style [ style.padding 20 ]
      prop.children [ Html.h1 [ prop.className "title"; prop.text "Elmish HackerNews" ]; renderItems state.StoryItems ] ]
