...
namespace MediaLibrary.Api.Helper;
    ...
    public MapperProfile()
    {
        ...
        CreateMap<Person, PersonModel>()
            .ForMember(
                x => x.MoviesIds,
                map => map
                    .MapFrom(actor => actor.Movies.Select(x => x.MovieId)));
        ...
        CreateMap<Movie, MovieModel>()
            .ForMember(
                x => x.Categories,
                map => map
                    .MapFrom(movie => movie.Categories.Select(x => x.Category)))
            .ForMember(
                x => x.ActorIds,
                map => map
                    .MapFrom(movie => movie.Actors.Select(x => x.PersonId)));
        ...
        CreateMap<MovieModel, Movie>()
            ...
            .ForMember(
                x => x.Actors,
                map => map
                    .MapFrom((model, entity) =>
                    {
                        var current = entity.Actors
                            .Select(x => x.PersonId)
                            .ToArray();
                        var actors = entity.Actors
                            .ToList();
                        actors.AddRange(model.ActorIds
                            .Where(x => !current.Contains(x))
