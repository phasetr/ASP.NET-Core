...
using WebApiMyBgList.GraphQL;
using WebApiMyBgList.gRPC;
using WebApiMyBgList.Models;
using WebApiMyBgList.Swagger;
...
// });
builder.Services.AddDistributedMemoryCache(options => options.SizeLimit = 50 * 1024 * 1024);
...
app.MapGet("/cod/test",
    [EnableCors("AnyOrig<in")] [ResponseCache(NoStore = true)]
    () =>
...
app.MapGet("/cache/test/2",
    [EnableCors("AnyOrigin")](HttpContext context) => Results.Ok());
